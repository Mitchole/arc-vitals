...
public class ArcVitalsOverlay extends Overlay {
    ...
    @Inject
    ArcVitalsOverlay(Client client, ArcVitalsConfig config) {
        this.client = client;
        this.config = config;
    ...
    public Dimension render(Graphics2D g) {
        ...
        drawVital(g, hp, pr, hpLeft, config.hpColor(), hpCur, hpMax, cx, cy);
        drawVital(g, pr, hp, !hpLeft, config.prayerColor(), prCur, prMax, cx, cy);
        return null;
    }

    private void drawVital(Graphics2D g, BarState self, BarState other, boolean leftSide,
                           Color baseColor, int current, int max, int cx, int cy) {
